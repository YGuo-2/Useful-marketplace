...
1. **受控入口：** 任务开始、完成、阻塞、跳过必须通过 `spec_progress.py` CLI 或 MCP 工具更新。
...
> 由 `python plugins/spec-coding/scripts/spec_progress.py waves docs/specs/` 计算。高风险任务即使可并行也必须单独成 wave。
