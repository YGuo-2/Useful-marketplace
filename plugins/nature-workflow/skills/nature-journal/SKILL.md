...
name: nature-journal
description: >-
  Select tiered target journals for a completed Nature-family manuscript, verify current journal facts
  on official sources, score 14-dimension fit, check 13 submission risks, and produce Tier 1 / Tier 2 /
  Tier 3 / Backup recommendations. After the user confirms a journal, study its recent 3-5 year
  articles across 8 style dimensions and verify author guidelines to build a submission-adaptation
  standard. Use for journal choice/comparison, submission risk, style/guideline learning, and Chinese
  triggers including 选刊、目标期刊选择、投哪个期刊、期刊匹配、分梯队选刊、冲刺/主投/稳妥/保底、
  投稿风险核查、期刊风格学习、作者指南核查、对标期刊学习、投稿格式适配. Routine journal style
  learning produces only an adaptation report. Only when the user explicitly asks to generate, save,
  or reuse a persistent target-journal 文风画像, prepare the verified corpus and hand profile
  generation and registration to nature-prose-style.
metadata:
  version: 0.2.0
...
- `../_shared/core/prose-profile-contract.md` — the explicit opt-in boundary and safe handoff contract
  for an optional persistent journal prose profile.
...
- **profile intent** — absent by default. Routine "learn this journal's style" or "adapt to this
  journal" requests do not create a persistent profile. Set this only when the user explicitly asks
  to generate, save, or reuse a journal prose profile.
...
Routine deep-learning ends with the adaptation report and must not write `prose-profiles/`, mutate
`nature.yml`, or install a host bootstrap. When and only when `profile intent` is explicit,
package the verified benchmark list and the abstracted language-style findings described by the output
contract, then hand them to `nature-prose-style` with `source_kind: journal-corpus`. That skill alone
generates, validates, registers, and selects the persistent profile.
