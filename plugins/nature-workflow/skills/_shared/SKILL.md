...
Do not route user requests here directly. Choose the relevant `nature-*` or
`openclaw-medical-skills` entrypoint, then read files from `_shared/` only when
that skill's instructions require them.
