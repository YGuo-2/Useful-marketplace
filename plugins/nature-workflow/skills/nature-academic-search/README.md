...
`nature-academic-search` 是面向 Claude Code / Codex 工作流的学术检索技能包，集成 CrossRef、PubMed、arXiv、OpenAlex、Europe PMC、Semantic Scholar、Scopus、ScienceDirect 检索源，以及 Unpaywall 开放获取全文。

## 功能

- **多源并发搜索**：默认查询 CrossRef、PubMed、arXiv、OpenAlex，并合并返回结果。
- **按 ID 获取详情**：支持 DOI、PMID、arXiv ID 自动识别。
- **格式化引用**：支持 APA、Nature、IEEE、Vancouver 等风格。
- **MeSH 词表查询**：辅助构建精准 PubMed 检索式。
- **开放获取全文**：通过 Unpaywall 按 DOI 查询合法开放获取 PDF / 落地页。
- **文献管理脚本**：支持 `.nbib`、`.ris`、`.bib`、`.enw` 格式互转。
- **Europe PMC / Semantic Scholar**：可选免费源，显式传入 `sources=["europepmc"]` / `["semanticscholar"]` 启用（Semantic Scholar 支持 `SEMANTIC_SCHOLAR_API_KEY` 提速）。
...
| 工具 | 说明 |
|------|------|
| `search_papers` | 默认四源（CrossRef、PubMed、arXiv、OpenAlex）并发搜索；可显式添加免费的 Europe PMC / Semantic Scholar 或 Scopus / ScienceDirect |
