...
class NatureMemoryTests(unittest.TestCase):
    ...
    def test_hash_titles_empty_metadata_and_locator_like_recall_are_unambiguous(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            repo = Path(tmp)
            workflow = make_workflow(repo)
            (workflow / "memory.md").write_text("## hash#title\nbody\n", encoding="utf-8")
            shown = nature_memory.command_memory_show(repo, workflow, "shared", "hash#title")
            self.assertTrue(shown["ok"], shown)
            touched = nature_memory.command_memory_touch(None, None, "hash#title", base=repo)
            self.assertTrue(touched["ok"], touched)

            empty = make_workflow(repo, "empty-metadata")
            (empty / "memory.md").write_text(
                "## empty\n<!-- nature-memory: {} -->\nbody\n", encoding="utf-8"
            )
            parsed = nature_memory.parse_memory_document((empty / "memory.md").read_text(encoding="utf-8"), empty / "memory.md")
            self.assertIn("missing_metadata_field", {item["code"] for item in parsed.diagnostics})

            canonical_workflow = make_workflow(repo, "canonical-locator")
            canonical = nature_memory.command_memory_remember(repo, canonical_workflow, "shared", "canonical", "body", {"kind": "decision"})
            mismatched = nature_memory.command_memory_recall(
                repo, canonical_workflow, "shared", f"other.md#{canonical['entry_id']}"
            )
            self.assertFalse(mismatched["ok"], mismatched)
            self.assertEqual(mismatched["error"]["code"], "locator_workflow_mismatch")

    def test_legacy_touch_preserves_unrelated_crlf_and_migrate_selector_conflicts_fail_closed(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            repo = Path(tmp)
            workflow = make_workflow(repo)
            original = "## M1 · first\r\nbody  \r\n\r\n## M2 · second\r\nsecond  \r\n\r\n"
            ...
            self.assertIn("## M2 · second\r\nsecond  \r\n\r\n", updated)
            self.assertIn("## M1 · first\r\n<!-- updated: ", updated)

            conflict = nature_memory.command_memory_migrate(repo, workflow, "shared", all_workflows=True, dry_run=True)
            self.assertFalse(conflict["ok"], conflict)
            self.assertEqual(conflict["error"]["code"], "conflicting_workflow_selector")

    @unittest.skipUnless(os.name == "nt", "Windows ReplaceFileW failure boundary")
    def test_replace_failure_is_structured_and_preserves_original_bytes(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            repo = Path(tmp)
            workflow = make_workflow(repo)
            created = nature_memory.command_memory_remember(repo, workflow, "shared", "replace", "body", {"kind": "decision"})
            before = (workflow / "memory.md").read_bytes()
            with patch.object(nature_memory, "_windows_atomic_replace", side_effect=PermissionError("forced")), patch.object(
                nature_memory, "_conditional_replace_posix", return_value=False
            ):
                result = nature_memory.command_memory_remember(
                    repo, workflow, "shared", "replace updated", "body", {"kind": "decision"},
                    entry_id=created["entry_id"], expected_etag=created["etag"],
                )
            self.assertFalse(result["ok"], result)
            self.assertEqual(result["error"]["code"], "replace_failed")
            self.assertEqual((workflow / "memory.md").read_bytes(), before)
    ...
    def test_migration_rejects_alias_title_collision_and_recovers_after_replace_failure(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch.object(
                ...
                before = (recover / "memory.md").read_bytes()
                failed = nature_memory.command_memory_migrate(repo, recover, "shared")
            self.assertFalse(failed["ok"], failed)
            self.assertEqual((recover / "memory.md").read_bytes(), before)
    ...
    def test_path_shaped_title_is_not_treated_as_locator(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            repo = Path(tmp)
            workflow = make_workflow(repo)
            (workflow / "memory.md").write_text("## foo/bar#baz\nbody\n", encoding="utf-8")

            shown = nature_memory.command_memory_show(repo, workflow, "shared", "foo/bar#baz")
            touched = nature_memory.command_memory_touch(None, None, "foo/bar#baz", base=repo)

            self.assertTrue(shown["ok"], shown)
            self.assertTrue(touched["ok"], touched)

    def test_canonical_touch_preserves_crlf_whitespace_and_trailing_layout(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            repo = Path(tmp)
            workflow = make_workflow(repo)
            canonical = nature_memory.serialize_entry(
                "canonical",
                "body  \n\ntrailing  ",
                schema_v1_metadata(),
            ).replace("\n", "\r\n")
            ...
            result = nature_memory.command_memory_touch(None, None, STABLE_ID, base=repo)
            after = path.read_bytes()
            before_lines = before.splitlines(keepends=True)
            after_lines = after.splitlines(keepends=True)

            self.assertTrue(result["ok"], result)
            self.assertEqual(before_lines[0:1] + before_lines[2:], after_lines[0:1] + after_lines[2:])
            self.assertEqual(after_lines[-1], b"\r\n")
            self.assertNotEqual(before_lines[1], after_lines[1])

    def test_hard_budget_error_includes_file_and_source_context(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            repo = Path(tmp)
            workflow = make_workflow(repo)
            created = nature_memory.command_memory_remember(
                repo, workflow, "shared", "budget", "body", {"kind": "decision"}
            )
            result = nature_memory.command_memory_remember(
                repo,
                workflow,
                "shared",
                "budget updated",
                "x" * nature_memory.HARD_FILE_BYTES,
                {"kind": "decision"},
                entry_id=created["entry_id"],
                expected_etag=created["etag"],
            )
            ...
            self.assertFalse(result["ok"], result)
            self.assertEqual(result["error"]["code"], "hard_file_budget")
            self.assertEqual(result["error"]["workflow_dir"], str(workflow))
            self.assertEqual(result["error"]["scope"], "shared")
            self.assertEqual(result["error"]["file_etag"], created["file_etag"])
            self.assertEqual(result["error"]["source_etags"][created["entry_id"]], created["etag"])

    def test_minimum_recall_budget_keeps_unsafe_diagnostic(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            repo = Path(tmp)
            workflow = make_workflow(repo)
            (workflow / "memory.md").mkdir()
            ...
            self.assertTrue(result["error"]["diagnostics"], result)
    ...
    def test_recall_scope_low_trust_and_sentinel_regressions_are_structured(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            repo = Path(tmp)
            workflow = make_workflow(repo)
            nature_memory.command_memory_remember(repo, workflow, "shared", "query", "body", {"kind": "decision"})

            invalid_scope = nature_memory.command_memory_recall(repo, workflow, "invalid", "query")
            control = nature_memory.command_memory_recall(repo, workflow, "shared", "bad\x01query")
            sentinel = nature_memory.command_memory_recall(repo, workflow, "shared", nature_memory.SENTINEL_START)

            self.assertEqual(invalid_scope["error"]["code"], "invalid_scope")
            self.assertEqual(control["error"]["code"], "control_character")
            self.assertEqual(sentinel["error"]["code"], "sentinel_injection")

    def test_compatibility_apis_report_missing_project_root(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            missing = Path(tmp) / "missing"

            results = [
                nature_memory.command_memory_check(base=missing),
                nature_memory.command_memory_list(base=missing),
                nature_memory.command_memory_touch(None, None, "M1", base=missing),
                nature_memory.command_memory_index(base=missing),
            ]

            self.assertEqual([item["error"]["code"] for item in results], ["project_root_not_found"] * 4)

    @unittest.skipUnless(os.name != "nt", "POSIX CAS fallback boundary")
    def test_posix_without_renameat2_fails_closed_without_overwriting(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            repo = Path(tmp)
            workflow = make_workflow(repo)
            created = nature_memory.command_memory_remember(repo, workflow, "shared", "cas", "body", {"kind": "decision"})
            before = (workflow / "memory.md").read_bytes()
            with patch.object(nature_memory, "_conditional_replace_posix", return_value=False):
                result = nature_memory.command_memory_remember(
                    repo,
                    workflow,
                    "shared",
            ...
            self.assertFalse(result["ok"], result)
            self.assertEqual(result["error"]["code"], "cas_unavailable")
            self.assertEqual((workflow / "memory.md").read_bytes(), before)

    def test_agents_backup_partial_failure_is_clean_and_retryable(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            repo = Path(tmp)
            make_workflow(repo)
            agents = repo / "AGENTS.md"
            agents.write_text("outer\n", encoding="utf-8")
            with patch.object(nature_memory.shutil, "copyfileobj", side_effect=OSError("forced")):
                failed = nature_memory.command_memory_index(base=repo)

            self.assertFalse(failed["ok"], failed)
            self.assertEqual(failed["error"]["code"], "agents_backup_failed")
            self.assertFalse((repo / "AGENTS.md.nature-memory.bak").exists())
            retried = nature_memory.command_memory_index(base=repo)
            self.assertTrue(retried["ok"], retried)

    def test_invalid_utf8_agents_is_structured(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            repo = Path(tmp)
            make_workflow(repo)
            agents = repo / "AGENTS.md"
    ...
    def test_all_workflow_migration_backup_oserror_is_structured_and_isolated(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            repo = Path(tmp)
            first = make_workflow(repo, "first")
            second = make_workflow(repo, "second")
            ...
            real_copy = nature_memory._copy_file_exclusive

            def fail_first(source: Path, destination: Path) -> None:
                if destination.parent.name == "first":
                    raise PermissionError("forced")
                real_copy(source, destination)

            with patch.object(nature_memory, "_copy_file_exclusive", side_effect=fail_first):
                result = nature_memory.command_memory_migrate(repo, scope="shared", all_workflows=True)
            ...
            self.assertEqual(by_workflow["first"]["error"]["code"], "migration_backup_failed")
            self.assertEqual(by_workflow["second"]["operation"], "migrated")

    def test_touch_locator_is_workflow_bound_and_terminal_entries_are_read_only(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            repo = Path(tmp)
            first = make_workflow(repo, "first")
            second = make_workflow(repo, "second")
            created = nature_memory.command_memory_remember(
                repo, first, "shared", "touch target", "body", {"kind": "decision"}
            )
            nature_memory.command_memory_remember(
    ...
    def test_nm_prefixed_unique_title_remains_resolvable(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            repo = Path(tmp)
            workflow = make_workflow(repo)
    ...
    def test_agents_parent_symlink_fails_closed_and_crlf_layout_is_preserved(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            outside.mkdir()
            (outside / "AGENTS.md").write_bytes(b"external\r\n")
            linked = repo / "linked"
            try:
                os.symlink(outside, linked, target_is_directory=True)
            except (OSError, NotImplementedError) as exc:
                self.skipTest(f"symlink unavailable: {exc}")
            escaped = nature_memory.command_memory_index(base=repo, agents_path="linked/AGENTS.md")
            self.assertFalse(escaped["ok"], escaped)
            self.assertEqual(escaped["error"]["code"], "path_symlink_escape")
            self.assertEqual((outside / "AGENTS.md").read_bytes(), b"external\r\n")
    ...
    def test_recall_budget_does_not_skip_a_high_ranked_oversized_record(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            repo = Path(tmp)
            workflow = make_workflow(repo)
            high = nature_memory.serialize_entry(
                "topic", "x" * 5000, schema_v1_metadata("nm_1234567890ab4cde8f0123456789abcd")
            )
            low = nature_memory.serialize_entry(
                "lower", "topic", schema_v1_metadata("nm_2234567890ab4cde8f0123456789abcd")
            )
            (workflow / "memory.md").write_text(high + low, encoding="utf-8")

            result = nature_memory.command_memory_recall(
                repo, workflow, "shared", "topic", top_k=2, max_bytes=4096
            )

            self.assertTrue(result["ok"], result)
            self.assertEqual(result["results"], [])

    def test_recall_all_dispatches_locator_to_its_workflow(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            repo = Path(tmp)
            first = make_workflow(repo, "first")
            second = make_workflow(repo, "second")
            nature_memory.command_memory_remember(repo, first, "shared", "first", "body", {"kind": "decision"})
            target = nature_memory.command_memory_remember(repo, second, "shared", "second", "body", {"kind": "decision"})

            result = nature_memory.command_memory_recall_all(
                repo, repo / "docs" / "nature-workflows", "shared", target["locator"]
            )

            self.assertTrue(result["ok"], result)
            self.assertEqual([item["id"] for item in result["results"]], [target["entry_id"]])

    @unittest.skipUnless(os.name != "nt", "POSIX CAS boundary")
    def test_posix_first_create_race_preserves_external_file(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            repo = Path(tmp)
            workflow = make_workflow(repo)
            memory_path = workflow / "memory.md"

            def appear_then_fail(source: str | os.PathLike[str], destination: str | os.PathLike[str]) -> None:
            ...
            self.assertFalse(result["ok"], result)
            self.assertEqual(result["error"]["code"], "file_changed_outside_lock")
            self.assertEqual(memory_path.read_bytes(), b"external\n")

    @unittest.skipUnless(os.name != "nt", "POSIX CAS boundary")
    def test_posix_unexpected_rename_errno_is_structured_with_etags(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            repo = Path(tmp)
            workflow = make_workflow(repo)
            created = nature_memory.command_memory_remember(
            ...
            with patch.object(nature_memory.ctypes, "CDLL", return_value=type("LibC", (), {"renameat2": FailingRename()})()):
                result = nature_memory.command_memory_remember(
                    repo,
                    workflow,
                    "shared",
    ...
    def test_all_workflow_migration_replacement_failure_cleans_backup_and_allows_retry(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertFalse(result["ok"], result)
            self.assertEqual(by_workflow["first"]["error"]["code"], "replace_failed")
            self.assertEqual((first / "memory.md").read_bytes(), before)
            self.assertFalse((first / "memory.md.nature-memory.bak").exists())
            self.assertEqual(by_workflow["second"]["operation"], "migrated")
