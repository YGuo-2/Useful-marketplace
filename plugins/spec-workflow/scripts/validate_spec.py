...
用法:
    python validate_spec.py <specs_dir>
    python validate_spec.py <specs_dir> --workflow requirements-first
    python validate_spec.py <specs_dir> --workflow design-first
    python validate_spec.py <specs_dir> --workflow bugfix
"""
...
COLOR_MODE = "auto"
VALID_RISK_VALUES = {"low", "medium", "high", "critical", "低", "中", "高", "低风险", "中风险", "高风险"}
VALID_PARALLEL_VALUES = {"true", "false", "yes", "no", "y", "n", "1", "0", "是", "否", "可", "不可", "parallel", "serial"}
...
DEFAULT_TEMPLATE_PATTERNS = [
    ...
    (re.compile(r"low\s*\|\s*medium\s*\|\s*high", re.IGNORECASE), "未替换风险等级选项"),
    (re.compile(r"风险\s*[:：]\s*(?:低\s*\|\s*中\s*\|\s*高|low\s*\|\s*medium\s*\|\s*high)", re.IGNORECASE), "未替换风险等级选项"),
    (re.compile(r"可并行\s*[:：]\s*是\s*\|\s*否"), "未替换并行选项"),
    (re.compile(r"\[命令或检查方法\]"), "未替换验证命令占位符"),
    (re.compile(r"\[如何验证[^\]]*\]"), "未替换验证标准占位符"),
    ...
    (re.compile(r"请根据实际(?:方案|架构|调用链)替换上图"), "未替换示例图提示"),
]
...
def check_task_graph(specs_dir: str, workflow: str) -> list[Result]:
    ...
    prefix = expected_task_prefix(workflow)
    for task in tasks:
        required = ("depends_on", "risk", "covers", "verify", "verify_criteria", "evidence", "parallelizable")
        missing = [field for field in required if not task.fields.get(field)]
        if missing:
            missing_required.append(f"{task.task_id}: {', '.join(missing)}")
        risk = task.fields.get("risk", "").strip().lower()
        if risk and risk not in VALID_RISK_VALUES:
            invalid_fields.append(f"{task.task_id}: risk={task.fields.get('risk')}")
        parallelizable = task.fields.get("parallelizable", "").strip().lower()
        if parallelizable and parallelizable not in VALID_PARALLEL_VALUES:
            invalid_fields.append(f"{task.task_id}: parallelizable={task.fields.get('parallelizable')}")
    ...
    if invalid_fields:
        preview = "; ".join(invalid_fields[:5])
        if len(invalid_fields) > 5:
            preview += f"; 另有 {len(invalid_fields) - 5} 项"
        results.append((False, f"tasks.md 结构化任务字段值非法: {preview}"))
    else:
        results.append((True, "tasks.md 风险和并行字段值合法"))
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Spec workflow 规范完整性验证工具",
        epilog="用于验证选定 <specs_dir> 目录下的 Requirements-First、Design-First 或 Bugfix 工件。",
    )
    parser.add_argument("specs_dir", help="规范文件所在目录路径 (如 docs/specs/<run-id>/)")
    parser.add_argument(
        "--workflow",
        default="auto",
    ...
    print(colorize("\n╔══════════════════════════════════════════════╗", Colors.CYAN))
    print(colorize("║   Spec workflow 规范完整性验证                ║", Colors.CYAN))
    print(colorize("╚══════════════════════════════════════════════╝\n", Colors.CYAN))

    specs_dir = args.specs_dir
    if not os.path.isdir(specs_dir):
        print(colorize(f"✖ 目录不存在: {specs_dir}", Colors.RED))
        print("\n请先运行 Spec workflow 工作流生成规范文件。")
        sys.exit(1)
